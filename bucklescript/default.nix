...
let
  bucklescript-src = {
    name = "BuckleScript-bucklescript";
  } // builtins.fromJSON (builtins.readFile ./bucklescript-src.json);

  # NOTE: this should match the submodule checkout used by bucklescript.
  ocaml-src = {
    name = "BuckleScript-ocaml";
  } // builtins.fromJSON (builtins.readFile ./ocaml-src.json);
in rec {
  ...
  ninja = stdenv.mkDerivation {
    ...
    postUnpack =
      "tar xzvf ${bucklescript-src.name}/vendor/ninja.tar.gz -C ${bucklescript-src.name}/vendor/ninja";
